...
class EditarMecanicoViewModel:
    def __init__(self, page: ft.Page, mecanico_id: int):
        ...
        logger.debug(f"ViewModel de Edição de Mecânico inicializado para o ID: {self.mecanico_id}")
    ...
    def carregar_dados(self):
        ...
        try:
            logger.info(f"ViewModel: buscando dados para o mecânico ID {self.mecanico_id}")
            mecanico = queries.obter_mecanico_por_id(self.mecanico_id)
            if mecanico:
                self._view.preencher_formulario(mecanico)
                logger.info(f"ViewModel: Dados do mecânico '{mecanico.nome}' carregados.")
            else:
                logger.warning(f"ViewModel: Mecânico com ID {self.mecanico_id} não encontrado.")
                acao_navegacao = lambda: self.page.go("/gerir_mecanicos")
                self._view.mostrar_dialogo_feedback("Erro", "Mecânico não encontrado.", acao_navegacao)
        except Exception as e:
            logger.error(f"Erro crítico ao carregar dados do mecânico: {e}", exc_info=True)
            if self._view:
                self._view.mostrar_dialogo_feedback("Erro Crítico", f"Não foi possível carregar os dados do mecânico.\nErro: {e}")
    ...
    def salvar_alteracoes(self):
        ...
        try:
            ...
            logger.info(f"ViewModel: salvando alterações para o mecânico ID {self.mecanico_id}")
            sucesso = queries.atualizar_mecanico(
                self.mecanico_id, nome, cpf, dados.get("especialidade", "").strip()
            )
            acao_navegacao = lambda: self.page.go("/gerir_mecanicos")
            
            if sucesso:
                self._view.mostrar_dialogo_feedback("Sucesso!", "Mecânico atualizado com sucesso!", acao_navegacao)
            else:
                self._view.mostrar_dialogo_feedback("Atenção", "Nenhuma alteração foi salva.")
        except sqlite3.IntegrityError:
            self._view.mostrar_dialogo_feedback("CPF Duplicado", f"Já existe outro mecânico cadastrado com o CPF '{cpf}'.")
        except Exception as e:
            logger.error(f"Erro crítico ao salvar alterações do mecânico: {e}", exc_info=True)
            if self._view:
