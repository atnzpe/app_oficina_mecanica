...
class DashboardViewModel:
    ...
    def __init__(self, page: ft.Page):
        ...
        self.usuario_atual: Usuario | None = self.page.session.get(
            "usuario_logado")

        if self.usuario_atual:
            logging.info(
                f"DashboardViewModel iniciado para o usuário: {self.usuario_atual.nome}")
        else:
            logging.warning(
                "DashboardViewModel iniciado sem um usuário na sessão.")
    ...
    def verificar_primeiro_cliente(self):
        """
        Verifica se existem clientes. Se não houver, navega para a tela de onboarding.
        """
        logging.info(
            "ViewModel: Verificando a existência de clientes para o fluxo de onboarding.")
        # A lógica de consulta ao banco permanece a mesma.
        if not queries.verificar_existencia_cliente():
            logging.info(
                "Nenhum cliente encontrado. Navegando para a rota de onboarding '/onboarding_cliente'.")

            self.page.go("/onboarding_cliente")
    ...
    def abrir_cadastro_cliente(self, e):
        """Navega o usuário para a página padrão de cadastro de cliente."""
    ...
    def abrir_cadastro_carro(self, e):
        ...
        if self._view:
            self._view.mostrar_feedback(
                "Funcionalidade 'Novo Veículo' a ser implementada.", True)
