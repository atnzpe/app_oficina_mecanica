...
class EditarClienteViewModel:
    def __init__(self, page: ft.Page, cliente_id: int):
        ...
        logger.debug(
            f"ViewModel de Edição de Cliente inicializado para o ID: {self.cliente_id}")
    ...
    def carregar_dados_cliente(self):
        ...
        if not self._view:
            logger.error(
                "ViewModel: Tentativa de carregar dados sem uma View vinculada.")
            return
        ...
        try:
            ...
            if cliente:
                ...
                logger.info(
                    f"ViewModel: Dados do cliente '{cliente.nome}' carregados e enviados para a View.")
            else:
                logger.warning(
                    f"ViewModel: Cliente com ID {self.cliente_id} não encontrado no banco.")
            ...
            logger.error(
                f"Erro crítico ao carregar dados do cliente: {e}", exc_info=True)
            self._view.mostrar_dialogo_feedback(
    ...
    def salvar_alteracoes(self, novos_dados: dict):
        ...
        try:
            ...
            if sucesso:
                logger.info(
                    f"ViewModel: Cliente ID {self.cliente_id} atualizado com sucesso.")
                self._view.mostrar_dialogo_feedback(
                    "Sucesso!", "Cliente atualizado com sucesso!", acao_navegacao)
            else:
                logger.warning(
                    f"ViewModel: Nenhuma linha foi alterada para o cliente ID {self.cliente_id}.")
                self._view.mostrar_dialogo_feedback(
                    "Atenção", "Nenhuma alteração foi salva. Os dados podem ser os mesmos.")
        except Exception as e:
            logger.error(
                f"Erro crítico ao salvar alterações do cliente: {e}", exc_info=True)
            self._view.mostrar_dialogo_feedback(
    ...
    def solicitar_desativacao_cliente(self):
        ...
        if self._view:
            logger.debug(
                f"ViewModel: Solicitando à View que mostre o diálogo de desativação.")
    ...
    def confirmar_desativacao_cliente(self):
        ...
        try:
            logger.info(
                f"ViewModel: Confirmando desativação do cliente ID: {self.cliente_id}")
            sucesso = queries.desativar_cliente_por_id(self.cliente_id)
            self._view.fechar_dialogo()
            def acao_navegacao(): return self.page.go("/gerir_clientes")
            if sucesso:
                logger.info(
                    f"ViewModel: Cliente ID {self.cliente_id} desativado com sucesso.")
                self._view.mostrar_dialogo_feedback(
                    "Sucesso!", "Cliente desativado com sucesso!", acao_navegacao)
            else:
                logger.error(
                    f"ViewModel: Falha ao desativar cliente ID {self.cliente_id} no banco.")
                self._view.mostrar_dialogo_feedback(
                    "Erro", "Erro ao desativar o cliente.")
        except Exception as e:
            logger.error(
                f"Erro crítico ao confirmar desativação: {e}", exc_info=True)
            self._view.mostrar_dialogo_feedback(
    ...
    def solicitar_ativacao_cliente(self):
        ...
        if self._view:
            logger.debug(
                f"ViewModel: Solicitando à View que mostre o diálogo de ativação.")
    ...
    def confirmar_ativacao_cliente(self):
        ...
        try:
            logger.info(
                f"ViewModel: Confirmando ATIVAÇÃO do cliente ID: {self.cliente_id}")
            sucesso = queries.ativar_cliente_por_id(self.cliente_id)
            self._view.fechar_dialogo()
            def acao_navegacao(): return self.page.go("/gerir_clientes")
            if sucesso:
                logger.info(
                    f"ViewModel: Cliente ID {self.cliente_id} ativado com sucesso.")
                self._view.mostrar_dialogo_feedback(
                    "Sucesso!", "Cliente ativado com sucesso!", acao_navegacao)
            else:
                logger.error(
                    f"ViewModel: Falha ao ativar cliente ID {self.cliente_id} no banco.")
                self._view.mostrar_dialogo_feedback(
                    "Erro", "Erro ao ativar o cliente.")
        except Exception as e:
            logger.error(
                f"Erro crítico ao confirmar ativação: {e}", exc_info=True)
            self._view.mostrar_dialogo_feedback(
