...
class EditarMecanicoView(ft.Column):
    def __init__(self, page: ft.Page, mecanico_id: int):
        ...
        self.controls = [
            ft.Text("Editando Mecânico", size=AppFonts.TITLE_MEDIUM,
                    weight=ft.FontWeight.BOLD),
            self._nome_field, self._cpf_field,
            self._endereco_field, self._telefone_field,  # NOVOS
            self._especialidade_field,
            ft.Row(
                [
                    ft.ElevatedButton(
                        "Cancelar", on_click=lambda _: self.page.go("/gerir_mecanicos")),
    ...
    def did_mount(self):
        logger.info(
            f"EditarMecanicoView montada para mecânico ID: {self.view_model.mecanico_id}. Carregando dados...")
        self.view_model.carregar_dados()

    def preencher_formulario(self, mecanico: Mecanico):
        logger.debug(
            f"View: Preenchendo formulário com dados de '{mecanico.nome}'.")
        self._nome_field.value = mecanico.nome or ""
        self._cpf_field.value = mecanico.cpf or ""
        self._endereco_field.value = mecanico.endereco or ""  # NOVO
        self._telefone_field.value = mecanico.telefone or ""  # NOVO
...
def EditarMecanicoViewFactory(page: ft.Page, mecanico_id: int) -> ft.View:
    return ft.View(
        ...
        controls=[ft.SafeArea(content=ft.Container(content=EditarMecanicoView(
            page, mecanico_id), alignment=ft.alignment.center, expand=True), expand=True)],
        padding=0
    )
