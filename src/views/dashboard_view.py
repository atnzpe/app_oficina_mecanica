...
class DashboardView(ft.Column):
    ...
    def __init__(self, page: ft.Page):
        ...
        self._dialogo_primeiro_cliente = ft.AlertDialog(
            ...
            content=ft.Text(
                "Percebemos que você ainda não tem clientes cadastrados. Deseja cadastrar o seu primeiro cliente agora?"
            ),
            actions=[
                # O botão "Sim" delega a ação de volta para o ViewModel.
                ft.ElevatedButton(
                    "Sim, vamos lá!", on_click=self.view_model.abrir_cadastro_cliente
                ),
        ...
        self.botoes_dashboard = {
            "ordem_servico": ft.ElevatedButton(
                ...
                on_click=self.view_model.abrir_form_os,
            ...
            "cadastrar_cliente": ft.ElevatedButton(
                "Novo Cliente",
                on_click=self.view_model.abrir_cadastro_cliente,
                icon=ft.Icons.PERSON_ADD,
                disabled=not is_logged_in,
            ),
            "cadastrar_carro": ft.ElevatedButton(
                "Novo Veículo",
                on_click=self.view_model.abrir_cadastro_carro,
                icon=ft.Icons.DIRECTIONS_CAR,
                disabled=not is_logged_in,
            ),
            "cadastrar_pecas": ft.ElevatedButton(
                "Nova Peça",
                on_click=self.view_model.abrir_cadastro_peca,
                icon=ft.Icons.ADD_SHOPPING_CART,
                disabled=not is_logged_in,
            ),
            "saldo_estoque": ft.ElevatedButton(
                "Verificar Estoque",
                on_click=self.view_model.abrir_saldo_estoque,
                icon=ft.Icons.INVENTORY_2,
                disabled=not is_logged_in,
            ),
            "relatorio": ft.ElevatedButton(
                "Gerar Relatórios",
                on_click=self.view_model.abrir_relatorios,
        ...
        if hasattr(gerir_clientes_component, "controls") and isinstance(
            gerir_clientes_component.controls[0], ft.ElevatedButton
        ):
        ...
        return ft.Card(
            content=ft.Container(
                ...
                content=ft.Column(
                    controls=[
                        ft.Text(
                            titulo, size=AppFonts.BODY_LARGE, weight=ft.FontWeight.BOLD
                        ),
...
def DashboardViewFactory(page: ft.Page) -> ft.View:
    ...
    return ft.View(
        route="/dashboard", controls=[dashboard_content], appbar=appbar, padding=10
    )
